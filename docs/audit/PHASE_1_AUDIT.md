...
**Follow-up (2026-08-19, raised by the project owner directly, not the review session):** closing this finding created a real gap — a `daily_tracking = false` goal was now excluded from the checklist *and* (once Slice 3 shipped) the whole engine, with nothing anywhere displaying it, so a user who created one had no way to ever see it again. Confirmed by grepping every `goals`-table query in the app before fixing: only the checklist and the engine's own fetch touch `goals`, and both correctly exclude these. Fixed with the minimum viable answer — a plain list on `/quests` (title, category, no interaction, no progress number), bounded to the same active-window check the engine uses, so an expired or not-yet-started overall goal doesn't show either. Explicitly **not** the "overall %" tracking ADR-001 gestures at — that's still undecided and still out of scope; this only stops the goal from being invisible. Verified against real local Postgres with three goals (daily-tracked, active overall-tracked, expired overall-tracked): checklist and the new list each contain exactly the one goal that belongs. 105/105 tests, tsc, lint, build clean. Commit `541d8f9`.
**Slice:** 2
