...
**Review note — VERIFIED FIXED (2026-08-18, round-4 review, probe against the committed engine, since removed):** the `activeGoalsOn` / `scheduledOn` split is the right cut, and `qualifyingDays` replacing raw `elapsed` holds up across every pattern probed, including the two the implementer flagged as untested:
- **zero-goal account:** `pct: 0`, `completed: false` at day 15, day 61 and day 152 (was `pct: 100`/`completed: true` at day 61). `streak: 0`.
- **all-goals-expired:** freezes at `pct: 17` (10 qualifying days ÷ 60) and stays there at 1 month and 5 months out — no longer climbs. `streak: 0`, because the walk now breaks on the goal-less tail instead of reporting a stale pre-expiry chain.
- **third pattern — goal-less gap mid-window** (goal A Jan1–20, nothing Jan21–Feb19, goal B from Feb20): `pct: 67` = exactly the 40 qualifying days ÷ 60, `graceUsed: 0`, no spurious window reset. The gap contributed nothing and neither did it corrupt the accounting on either side. `streak: 20` — correctly bounded by the gap rather than silently walking through it to add goal A's earlier run.
- **third pattern — staggered overlapping goals** (A Jan1–31, B Jan15–Feb28): `pct: 98` = 59 qualifying days ÷ 60. Union-of-active-goals, not double-counted per goal.
- **weekly-only user still accrues:** `pct: 100` after 90 days. This is the load-bearing case — a weekly goal is *active* every day even when only *due* weekly, so the C4 fix does not re-break C1. Confirmed it doesn't.
- **pause still excluded:** 7 paused days correctly contribute no qualifying days.

No third reading of `null` was introduced in `streak`/`rankProgress` — the conflation is genuinely resolved in both. It does however survive in `personalDevelopmentScore`; see C5.
...
### C5. `personalDevelopmentScore` still scores an empty account 20/100
**Status:** FIXED — resolved the same way as C4: `personalDevelopmentScore` now checks `activeGoalsOn(goals, today)` directly, and when it's empty, excludes the daily term entirely and renormalizes over rank+streak (weights `0.5/0.8`, `0.3/0.8`) rather than defaulting to 0 or 100. Documented in ADR-002 addendum §C. New tests: zero-goal account scores `0` (was `20`); all-goals-expired account renormalizes over rank+streak only (matches `round((0.5*rank + 0.3*streak) / 0.8)`, not a fixed 20-point daily floor). 81/81 green. Commit `2ac30a1`.
**Review note — VERIFIED FIXED (2026-08-18, round-4 review, probe since removed):**
- **zero-goal account: PDS `0`** (was 20). The 20-point floor is gone.
- **all-goals-expired: PDS `11`** (was 29) — rank frozen at 17, streak 0, renormalized over the remaining weights. Non-zero is correct here: 17% of rank progress was genuinely earned before the goals lapsed.
- **rest day preserved:** a weekly goal on a non-due day still takes the full formula with `daily = 100`, *not* the renormalized path — confirming the fix distinguishes the two `null` cases rather than replacing one blanket rule with another. This was the main way this fix could have gone wrong and it didn't.
- **ceiling holds:** the renormalized branch maxes at exactly `100`, no overflow past it.
- `remainingWeight` is derived from `PDS_WEIGHTS` rather than a hardcoded `0.8`, so changing the weights can't silently desynchronize the renormalization.

**Observation, deliberately not filed as a finding:** there is a one-day lag in `streak` at the moment a user's last goal expires. Probing daily across an expiry on `2026-03-01`: PDS/streak read `100`/`60` on Mar 1 (last active day), still `100`/`60` on Mar 2, then `63`/`0` from Mar 3 onward. Mar 2 still reports the 60-day streak because `streak` steps back one day when today isn't 100%, and that step lands on Mar 1, which was active. So a user with no active goals shows a live streak for exactly one day after expiry, then it drops. Not filed because the underlying numbers aren't wrong — the streak genuinely ended, and the eventual drop is correct, not a cliff in the accounting — and a one-day grace at expiry is arguably the kinder reading anyway. Recorded only so it isn't rediscovered as a bug later, and so that if Phase 1's dashboard ever animates or diffs these values, the transition is known to be non-monotonic by one day.
**Finding:** C4's fix taught `streak` and `rankProgress` to distinguish "no active goals" from "goals active, nothing due today". `personalDevelopmentScore` still collapses both into `100` via `daily ?? 100`, so the daily component pays out full marks to an account that has nothing to complete. With the 0.2 daily weight that is a floor of 20 on the whole score. Measured after the C4 fix:
- **zero-goal account:** rank 0, streak 0, daily `null`→100 ⇒ **PDS 20** (was 70 before C4 — much better, still not 0).
- **all-goals-expired account:** rank 17, streak 0, daily `null`→100 ⇒ **PDS 29**.

So the dashboard's headline "Overall development score" tile would read 20 for a user who has never created a goal, and 29 for one whose goals lapsed months ago. Materially less wrong than C4 (no false claim of rank completion) and *not* a launch blocker, but it is the same conflation C4 was about, surviving in the one place the fix didn't touch — and it is the number the PRD puts most prominently in front of the user.

The `?? 100` is defensible for the case it was written for: on a genuine rest day (goals active, nothing due) the user has completed everything asked of them, and 100 is right. It is only wrong when there was nothing to ask.
**Guardrail:** Make the same distinction the other two functions now make — when `activeGoalsOn(goals, today).length === 0`, either return 0 outright or drop the daily term and renormalize the remaining weights, rather than treating "nothing to do" as "did everything". **General rule: when a fix corrects how one sentinel is interpreted, grep for every other place that sentinel is consumed and fix them in the same change — C4 fixed two of three call sites, and the third kept the old behavior quietly because no test covered it.** Add a zero-goal and an all-expired case to the PDS tests, mirroring the ones C4 added for `rankProgress`.
...
**Status:** FIXED — both halves closed.
**Where:** `src/lib/schemas/*.test.ts` (new), `src/lib/supabase/rls.integration.test.ts` (new), `vitest.config.integration.mts` (new), `.github/workflows/ci.yml` (`rls-tests` job).
**Finding, and what was done:**
- **Zod schema validation** (commit `4b3d3be`): 40 new tests across `common.test.ts`, `goal.test.ts`, `rank-window.test.ts` — the `targetDate >= startDate` refinement (both `goalSchema` and `createGoalSchema`), the paired-null pause refinement, `pausedUntil >= pausedFrom`, and S3's `rankTargetSchema` (rejects `"E"`) vs `rankSchema` (accepts it) split. Fits the existing pure-unit `vitest` setup directly.
- **RLS isolation** (commits `fdedac5`, `d06756b`, `efa71db`): real signups via `@supabase/supabase-js` against a running local Supabase, exercising the actual client → PostgREST → RLS path rather than hand-crafted SQL. Chosen over pgTAP specifically to avoid guessing at `auth.users`' internal schema. Kept out of the fast suite via a separate `vitest.config.integration.mts` (`npm run test:rls`) and a separate `rls-tests` CI job with a Docker-backed local stack — matching the guardrail below. Refuses to run against anything but a local URL, since it creates throwaway users it never cleans up.
...
**Review note — VERIFIED FIXED (2026-08-18, round-4 review):** re-ran the original bypass table against `isLocalSupabaseUrl` plus new cases — **16/16 correct.** The userinfo bypass (`http://localhost:54321@evil.com/`) is now REFUSED, as are `http://127.0.0.1@evil.com/` and `http://localhost@evil.com:54321/`. Both cosmetic false-refusals are gone: `http://LOCALHOST:54321` and `http://[::1]:54321` now ALLOW. Also confirmed the parser handles alternate loopback spellings correctly rather than by accident — `http://2130706433/` and `http://127.1/` both normalize to `127.0.0.1` and are allowed (genuinely local), while `http://0.0.0.0/` is refused, and a URL merely *containing* the string localhost in its query or fragment (`https://evil.com/?h=localhost:54321`, `https://evil.com/#localhost`) is refused — which is precisely the class of mistake the old regex was prone to. Extracting it into its own module with its own unit tests also means this is now regression-protected rather than resting on a re-audit.
